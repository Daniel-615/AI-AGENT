...
func a_estrella(
	...
	while abiertos.size() > 0:
		...
		for direccion in direcciones:
			...
			var penalizacion_aprendida = (
				memoria_peligro.get(vecino, 0)
			)
			
			var tentative_g = (
				g_score.get(actual, INF)
				+ costo_base
				+ costo_peligro
				+ penalizacion_aprendida
			)
